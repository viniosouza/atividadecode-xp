...
function listarProdutos() {
  ...
  for (var i = 0; i < produtos.length; i++) {
    ...
    templete += '<input id=' + produtos[i].codigo + '  onChange="atualizarQuantidade(this.id, this.value)" type="number" value=' + produtos[i].qtd + '>';
    templete += '<button>-</button>';
    templete += '</td>';
...
function atualizarQuantidade(_codigo, _qtd) {
  var totalGeral = console.log(document.getElementById('Ttotal-geral').innerHTML);
  for (var index = 0; index < produtos.length; index++) {
    if (produtos[index].codigo == _codigo) {
      ...
      var totalAntigo = parseFloat(document.getElementById(codPro).innerHTML6)
      var totalProduto = produtos[index].preco * produtos[index].qtd
      var codPro = 'total-' + produtos[index].codigo
      document.getElementById(codPro).innerHTML = totalGeral;
